...
fixed fixed::sin() const {
    ...
    const fixed x2 = x * x;
    return sign * x *
               (fixed::PI - x2 * (fixed::TAU - fixed::from_int(5) -
                                  x2 * (fixed::PI - fixed::from_int(3)))) >>
           1;
}

fixed fixed::cos() const { return (*this + fixed::PI_DIV_2).sin(); }

fixed fixed::tan() const {
    fixed cx = cos();
    ERR_FAIL_COND_V_MSG(cx == fixed::ZERO, fixed::ZERO,
                        "tan() of 90 degree angles is undefined");
    return sin() / cx;
}
...
fixed fixed::pow(const fixed& exp) const {
    if (value == 0) {
        ERR_FAIL_COND_V_MSG(exp.value < 0, fixed::ZERO,
                            "Cannot use negative exponents and a base of 0 "
                            "with integer exponentiation!");
        return fixed::ZERO;
    }
