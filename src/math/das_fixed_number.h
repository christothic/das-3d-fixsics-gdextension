...
struct fixed {
    ...
    _FORCE_INLINE_ fixed floor() { return fixed(value & ~0xFFFF); }

    _FORCE_INLINE_ fixed round() { return (*this + fixed::HALF).floor(); }

    _FORCE_INLINE_ fixed ceil() {
        return (value & 0xFFFF) ? (*this + fixed::ONE).floor() : *this;
    }

    _FORCE_INLINE_ int64_t to_int() const { return value >> 16; }

    _FORCE_INLINE_ float to_float() const { return (float)value / 65536.0f; }

    _FORCE_INLINE_ fixed operator+(const fixed& p_other) const {
#ifdef DAS_FIXED_MATH_CHECKS
        ERR_FAIL_COND_V_MSG(
            p_other.value > 0 && (value > INT64_MAX - p_other.value),
            fixed::ARITHMETIC_OVERFLOW, "Fixed-point addition overflow");
        ERR_FAIL_COND_V_MSG(
            p_other.value < 0 && (value < INT64_EFFECTIVE_MIN - p_other.value),
            fixed::ARITHMETIC_OVERFLOW, "Fixed-point addition overflow");
#endif
    ...
    _FORCE_INLINE_ fixed sign() const {
        return value < 0 ? fixed::NEG_ONE
                         : (value > 0 ? fixed::ONE : fixed::ZERO);
    }
    _FORCE_INLINE_ fixed move_toward(const fixed& p_other, fixed p_delta) {
        return (p_other - *this).abs() <= p_delta
                   ? p_other
